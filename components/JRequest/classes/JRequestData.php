...
//        return '<a class="fancybox" href="'.$file.'" title="Нажми для просмотра"><img src="'.$file.'" height="50px"></a>';
    ...
    private function getApprovedButton($id) {
        $out = "";
        if($this->yourUser->isAdmin() && $this->JRequestDate['Request'][$id]['preapproved']>0) {
            $approvedParam[0]=$this->fest;
            $approvedParam[1]=$id;
            if(!$this->JRequestDate['Request'][$id]['approved']>0) {
                $JRequestCheck = new JRequestCheck($id);
                if($JRequestCheck->getCheckStatus()) {
                    $approvedParam[2]='sendApproved';
                    $out .= '<a href="'.$this->urlHelper->chengeParams($approvedParam).'">';
                    $out .= '<input class="JRequestFormButton small" type="button" value="Одобрить заявку">';
                    $out .= '</a>';
                }
            } else {
                $approvedParam[2]='sendApproved';
                $approvedParam[3]='unset';
                $out .= '<a href="'.$this->urlHelper->chengeParams($approvedParam).'">';
                $out .= '<input class="JRequestFormButton small" type="button" value="Снять с одобрения">';
                $out .= '</a>';
            }
        }
        return $out;
