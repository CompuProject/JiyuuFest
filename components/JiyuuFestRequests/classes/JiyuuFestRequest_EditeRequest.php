<?php
/**
 * Description of JiyuuFestRequest_EditeRequest
 *
 * @author maxim
 */
class JiyuuFestRequest_EditeRequest {
    // помошники
    ...
    protected function deletFiles() {
        if(isset($this->insertData['deletFile'])) {
            foreach ($this->insertData['deletFile'] as $deletFile) {
                $query = "SELECT `".$deletFile."` FROM `".$this->requestTypeData['tableName']."` WHERE `request`='".$this->requestID."';";
                $rezult = $this->SQL_HELPER->select($query,1);
                $fileName = $rezult[$deletFile];
                if(file_exists($this->fileDir.$fileName)) {
                    unlink($this->fileDir.$fileName);
                }
                $query = "UPDATE `".$this->requestTypeData['tableName']."` SET `".$deletFile."`=NULL WHERE `request`='".$this->requestID."';";
                $this->SQL_HELPER->insert($query);
            }
        }
    }
